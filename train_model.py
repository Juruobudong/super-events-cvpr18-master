...
if args.dataset == 'multithumos':
    ...
    rgb_root = '/gpfs/home/lianghe/xuexinwei/xxw/super-events-cvpr18-master/data/charades/charades_rgb_features'
    flow_root = '/gpfs/home/lianghe/xuexinwei/xxw/super-events-cvpr18-master/data/charades/charades_flow_features'
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        print 'Evaluating...'
        print args.rgb_model_file
        if(args.rgb_model_file.strip().split('/')[-1]=='cvpr_charades_rgb_final'):
            rgb_model = super_event.get_super_event_model(0, classes)
            print rgb_model
            rgb_model = rgb_model.load_state_dict(torch.load(args.rgb_model_file))
            rgb_model.cuda()
            print rgb_model
            ...
            flow_model = super_event.get_super_event_model(0, classes)
            flow_model = flow_model.load_state_dict(torch.load(args.flow_model_file))
            flow_model.cuda()
            dataloaders, datasets = load_data('', test_split, flow_root)
            flow_results = eval_model(flow_model, dataloaders['val'])
        elif (args.rgb_model_file.strip().split('/')[-1]=='rgb_baseline'):
